...
class MaskHierarchicalAttLeToR(HierarchicalAttLeToR):
    def _align_to_rank_model(self, l_inputs, l_models):
        l_aligned_models = [model(input) for model, input in zip(l_models, l_inputs)]
        ranker_model = Merge(mode='concat', concat_axis=1, name='rank_merge')(l_aligned_models[:2])
        att_model = Merge(mode='concat', concat_axis=1, name='att_merge')(l_aligned_models[2:])
        att_ranker = Merge(mode='dot', dot_axes=1, output_shape=(1,))([ranker_model, att_model])
        att_ranker = Model(input=l_inputs, output=att_ranker)
        return att_ranker
